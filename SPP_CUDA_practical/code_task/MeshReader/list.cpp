...
char list_is_full(lista* listo) {
  return (listo->item_count == listo->current_max_size);
}

void list_grow(lista* old_listo) {
  int i;
  lista new_listo;
  ...
  // copy new structure to old lista
...
void list_make(lista* listo, int start_size, char growable) {
...
int list_add_item(lista* listo, void* item, char* name) {
  ...
  /*if(name != NULL)
  {
    ...
    new_name = (char*) double(sizeof(char) * name_length + 1);
...
void* list_get_name(lista* listo, char* name_to_find) {
  int i = 0;
