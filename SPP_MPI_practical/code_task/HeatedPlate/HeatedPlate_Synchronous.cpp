...
#define N 500
    ...
    while (epsilon <= diff) {
        ...
        //Send and receive necessary rows to finish remaining nodes
        //MPI_Sendrecv is used, so no deadlock can happen
        if(rank == 0) //does only need row below and must send the lowest row only
        {
            double below[N];
            MPI_Sendrecv(u[rows-1], N, MPI_DOUBLE, rank+1, 0, below, N, MPI_DOUBLE, rank+1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            //calc remaining row
            for (j = 1; j < N - 1; j++)
            {
        ...
        {
            double above[N];
            MPI_Sendrecv(u[0], N, MPI_DOUBLE, rank-1, 0, above, N, MPI_DOUBLE, rank-1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            //calc remaining row
            for (j = 1; j < N - 1; j++)
            {
